...
const hook: Hook<'init'> = async function (opts) {
  ...
  process.env.XDG_CACHE_HOME ? initCache(process.env.XDG_CACHE_HOME, activeSubscription) : initCache(global.config.cacheDir, activeSubscription)
  // process.env.XDG_CACHE_HOME ? console.log('XDG_CACHE_HOME') : console.log('this.config.cacheDir')
  process.env.XDG_CONFIG_HOME ? initConfig(process.env.XDG_CONFIG_HOME) : initConfig(global.config.configDir)

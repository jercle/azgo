...
class ConfigDevopsDefaultSub extends AzureDevOpsCommand {
    async run() {
...
ConfigDevopsDefaultSub.description = 'Shows the default subscription for Azure DevOps';
ConfigDevopsDefaultSub.examples = [
    '<%= config.bin %> <%= command.id %>',
    '<%= config.bin %> <%= command.id %> --select',
];
ConfigDevopsDefaultSub.flags = {
...
ConfigDevopsDefaultSub.args = {
    file: Args.string({ decription: "file to read" })
};
export default ConfigDevopsDefaultSub;

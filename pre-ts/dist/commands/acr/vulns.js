...
import { transformVulnerabilityData, groupByAttribute, getAllUniqueCves, countByAttribute } from '../../funcs/azureVulnarabilityAggregation.js';
// const activeSubscription = JSON.parse(readFileSync(`${homedir()}/.azure/azureProfile.json`)
//   .toString()
//   .trim())
//   .subscriptions
//   .filter(sub => sub.isDefault)[0]
class AcrVulns extends AzureCommand {
    async run() {
        ...
        if (!opts.acrRegistry) {
            if (containerRegsitries.length === 1) {
                ...
                opts.acrRegistryId = containerRegsitries[0].id;
...
AcrVulns.description = 'Get all vulnerabilities related to container images';
AcrVulns.examples = [

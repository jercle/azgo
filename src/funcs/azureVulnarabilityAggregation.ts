...
export function transformVulnerabilityData(vulns: any, repos = null) {
  // console.log(vulns)
...
export function getAllManifests(repos: any, filter = null) {
  const allTaggedManifests = repos.reduce((all: any, item: any, index: any) => {
    const manifests = item.manifests.map((manifest: any) => {
  ...
  return allTaggedManifests.reduce((all: any, item: any, index: any) => {
    all[item.digest] = {
      repo: item.repo,
      tags: item.tags
    }
...
export function groupByRepoUnderCve(data) {
  return data.reduce((all: any, item: any, index: any) => {
    const cves = item.cves.map(cve => cve.title)
    cves.forEach(cve => {
      if (!all[cve]) {
        all[cve] = {}
...
export function groupByAttribute(data, attr: string) {
  if (attr.toLowerCase() === 'imagetags' || attr.toLowerCase() === 'tags') {
    ...
    let allCves = cvesWithTags.reduce((all: any, item: any, index: any) => {
...
export function countByAttribute(data, attribute: string, dataType: string) {
  ...
  if (dataType === 'object') {
    ...
    const tags = Object.keys(grouped).reduce((all: any, item: any, index: any) => {
